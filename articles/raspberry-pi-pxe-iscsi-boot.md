...
- **SSD の耐久性**: NAS の SSD はエンタープライズ向けの耐久性を持ち、書き込み寿命の心配が大幅に減る
...
| ノード 1 (pi-1) | Raspberry Pi 4B | シリアル: `6f671c54` |
| ノード 2 (pi-2) | Raspberry Pi 4B | シリアル: `0d07dd2c` |
| ノード 3 (pi-3) | Raspberry Pi 4B | シリアル: `d829fe8c` |
...
graph TB
    ...
    Pi1["Pi #1<br/>6f671c54"]
    Pi2["Pi #2<br/>0d07dd2c"]
    Pi3["Pi #3<br/>d829fe8c"]
...
services:
    ...
    nfs-server:
        ...
        privileged: true
        environment:
        - NFS_EXPORT_0=/nfsshare/tftproot/d829fe8c 192.168.0.67(rw,async,no_subtree_check,no_root_squash,insecure)
        - NFS_EXPORT_1=/nfsshare/tftproot/0d07dd2c 192.168.0.202(rw,async,no_subtree_check,no_root_squash,insecure)
        - NFS_EXPORT_2=/nfsshare/tftproot/6f671c54 192.168.0.2(rw,async,no_subtree_check,no_root_squash,insecure)
        volumes:
        - ./tftproot/d829fe8c:/nfsshare/tftproot/d829fe8c
        - ./tftproot/0d07dd2c:/nfsshare/tftproot/0d07dd2c
        - ./tftproot/6f671c54:/nfsshare/tftproot/6f671c54
        network_mode: host
```
...
dhcp-host=dc:a6:32:93:70:06,set:raspberrypi-pxe  # pi-3
dhcp-host=dc:a6:32:93:6f:db,set:raspberrypi-pxe  # pi-2
dhcp-host=dc:a6:32:93:52:30,set:raspberrypi-pxe  # pi-1
...
Raspberry Pi 4B でネットワークブートを有効にするには、OTP ビットを設定する必要があります。**一度設定すると元に戻せない**ため、慎重に実行してください。
