...
実際に NFS root と iSCSI root の両方で UnixBench を実行したところ、iSCSI の方が総合スコアで約 23% 高い結果が出ました（詳細は[パフォーマンスと運用知見](#パフォーマンスと運用知見)で後述します）。特にシェルスクリプト実行のようなメタデータ操作が多いワークロードで大幅な改善が見られたため、iSCSI を採用することにしました。
...
PXE + iSCSI 構成に移行してから数ヶ月が経過した時点での所感とパフォーマンスデータをまとめます。
