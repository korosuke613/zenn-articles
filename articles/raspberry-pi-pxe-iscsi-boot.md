...
※ CPU 使用率は環境やワークロードにより異なります[^cpu-usage]。

[^cpu-usage]: [A Comparison of NFS and iSCSI for IP-Networked Storage (UMass FAST'04)](https://lass.cs.umass.edu/papers/pdf/FAST04.pdf)
...
`dhcp-reply-delay=2` は、Raspberry Pi の PXE クライアントのファームウェアバグを回避するために必要です[^dhcp-reply-delay]。この遅延がないとブートループに陥る場合があります。

[^dhcp-reply-delay]: [Raspberry Pi firmware issue #934](https://github.com/raspberrypi/firmware/issues/934)
:::
...
# 中身は空でよい（フラグファイル）。詳細: https://github.com/open-iscsi/open-iscsi/blob/master/debian/README.Debian
mkdir -p /etc/iscsi
...
# ユーザー作成とパスワード設定
# Bookworm 以降はデフォルトの pi ユーザーが存在しないため、手動で作成する
# ユーザー名とパスワードはお好みで
