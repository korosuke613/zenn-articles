...
SD カードの課題を受けて、そこら辺に転がっていた外付け HDD に OS を入れて USB 接続（micro USB Type-B）して運用していました。しかし、micro USB Type-B の接続不良が発生しやすく、Pi を移動するたびにケーブルが抜けるリスクがありました。各 Pi に HDD とケーブルがぶら下がる状態も取り回しが悪く、長期運用には向いていませんでした。
